...
if len(sys.argv)>1:
    ...
    sys.exit("usage: python ego_tgmod.py <CONFIGURATION>.cfg (integer restart-code, optional)")
...
try:
    ...
    logging.info("Connection to server successfull!")
except:
...
INSERT INTO _db_status (module, status) VALUES ('grid_model', FALSE);
    ...
    scripts = ['sql-scripts/extensions.sql', 
               ...
               'sql-scripts/grid_development.sql', 
...
if not config_continue_run:
    logging.info("Setting apply changes...")
    cur.execute("SELECT otg_apply_changes(%s, %s);", (v_plan_ids, v_year))
    conn.commit()	
...
if not config_continue_run: ##dk debugging - to be removed
    cur.execute("drop table if exists dk_debug;create table dk_debug (step_before int,max_bus_id int, num_bus int,max_branch_id int, num_branch int)") ##dk debugging - to be removed
    conn.commit() ##dk debugging - to be removed
...
for i,command in enumerate("'".join([segment.replace(";","§") if i%2==0 else segment for i,segment in enumerate(sqlfile_without_comments.split("'"))]).split("§")[:-1]):
    if i>=config_continue_run_at:
        ...
        try:
            ...
            logging.exception("Exception raised with command {0}. Check data and code and restart with 'python ego_tgmod.py {1} {0}'.".format(i,config_database))
            sys.exit()
        if i>16:##dk debugging - to be removed
            cur.execute("insert into dk_debug values ({0},(select max(id) from bus_data),(select count(*) from bus_data),(select max(branch_id) from branch_data),(select count(*) from branch_data))".format(i))##dk debugging - to be removed
            conn.commit()##dk debugging - to be removed
    
logging.info('Power-script executed successfully.')

logging.info("Saving Results...")
cur.execute("SELECT otg_save_results (%s, %s, %s);", (v_plan_ids, v_year, comment))
conn.commit()
...
tables = ['bus_data', 'branch_data', 'dcline_data', 'substations', 'problem_log', 'plz_subst', 'nuts3_subst','results_metadata']
for table in tables:
